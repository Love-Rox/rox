...
1. **NoteService実装**
   - ✅ ノート作成（`services/NoteService.ts`）
   - ✅ テキストまたはファイル必須バリデーション
   - ✅ ファイル数制限（最大4ファイル/ノート）
   - ✅ ファイル所有権確認
   - ✅ リプライ・Renote先存在確認
   - ✅ メンション抽出（@username）
...
2. **タイムライン実装**
   - ✅ ローカルタイムライン（公開投稿のみ）
   - ✅ ホームタイムライン（フォロー中のユーザー）
   - ✅ ユーザータイムライン（特定ユーザーの投稿）
   - ✅ ページネーション対応（limit, sinceId, untilId）
   - ✅ デフォルト20件、最大100件
...
4. **セキュリティ**
   - ✅ 作成時は認証必須（requireAuth）
   - ✅ タイムライン取得はオプショナル認証（optionalAuth）
   - ✅ 削除時の所有者確認
   - ✅ ファイル所有権確認
...
5. **機能サポート**
   ...
   - ✅ ファイル添付（最大4ファイル）
   - ✅ Content Warning（CW）
   - ✅ Visibility設定（public, home, followers, specified）
   - ✅ ローカルオンリー（連合無効化）
   - ✅ リプライ（replyId）
   - ✅ Renote（renoteId）
...
- ⏳ Renote専用エンドポイント（/api/notes/renote）
- ⏳ リプライ一覧取得（/api/notes/replies）
- ⏳ Renote一覧取得（/api/notes/renotes）
- ⏳ 会話スレッド取得（/api/notes/conversation）
...
1. **ReactionService実装**
   - ✅ リアクション作成（`services/ReactionService.ts`）
   - ✅ 既存リアクションの自動置換（1ユーザー1リアクション/ノート）
   - ✅ リアクション文字列長バリデーション（最大100文字）
   - ✅ ノート存在確認
   - ✅ リアクション削除
   - ✅ リアクション集計機能
   - ✅ 複数ノートの一括集計
...
2. **リアクションAPIエンドポイント**
   - ✅ `POST /api/notes/reactions/create` - リアクション作成・更新
   - ✅ `POST /api/notes/reactions/delete` - リアクション削除
   - ✅ `GET /api/notes/reactions` - ノートのリアクション一覧
   - ✅ `GET /api/notes/reactions/counts` - リアクション集計
   - ✅ `GET /api/notes/reactions/my-reaction` - 自分のリアクション取得

3. **機能サポート**
   - ✅ Unicode絵文字（👍、❤️など）
   - ✅ カスタム絵文字（:custom_emoji:）
   - ✅ リアクション重複防止（1ユーザー1リアクション）
   - ✅ 絵文字別集計（{ "👍": 5, "❤️": 3 }）

4. **セキュリティ**
   - ✅ 作成・削除時は認証必須（requireAuth）
   - ✅ 一覧・集計はオプショナル認証（optionalAuth）
   - ✅ 削除時の所有者確認
