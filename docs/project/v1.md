...
| **Styling** | **Tailwind CSS** | ビルド時のサイズ最適化に優れたユーティリティファーストCSS。 |
...
* [ ] Bun + Hono プロジェクトの初期化（モノレポ構成推奨）。
* [ ] Drizzle ORMのセットアップ。
  * PostgreSQL, MySQL, SQLite それぞれのスキーマ定義とマイグレーション環境の構築。
* [ ] 環境変数 (`.env`) によるDB/ストレージの切り替えロジック実装。
* [ ] 依存性注入 (DI) 機構の構築（Hono Contextの活用）。

### Phase 2: Misskey互換API実装 (Local API)

* [ ] **認証 (MiAuth):** セッション生成、認証可否の確認。
* [ ] **アカウント管理:** ユーザー登録、プロフィール編集 (`/api/i/update`)。
* [ ] **ノート機能:**
  * 投稿作成 (`/api/notes/create`): テキスト、画像添付、CW、公開範囲。
  * タイムライン取得 (`/api/notes/local-timeline`): ページネーション対応。
  * リアクション (`/api/notes/reactions/create`): 絵文字の付与。
* [ ] **ファイル管理:** ドライブ機能 (`/api/drive/*`) の最小実装。

### Phase 3: フロントエンド実装 (Waku Client)

* [ ] Waku + Jotai 環境構築。
* [ ] **共通コンポーネント:** Tailwind CSSを用いたUIキット作成。
* [ ] **認証フロー連携:** MiAuthによるログイン処理。
* [ ] **タイムライン描画:** Server Components (RSC) を活用した高速な初期表示。
* [ ] **投稿インタラクション:** Client Components による動的な投稿・リアクション処理。

### Phase 4: ActivityPub 連合 (Federation)

* [ ] **Actor実装:** `/users/:id` での JSON-LD (Person) 返却。
* [ ] **WebFinger:** `/.well-known/webfinger` 実装。
* [ ] **署名処理:** HTTP Signatures (RSA-SHA256 / Ed25519) の実装。
* [ ] **配送システム:**
  * Inbox (`/users/:id/inbox`) でのアクティビティ受信と検証。
  * Job Queue (Dragonfly/BullMQ) を用いた外部サーバーへの非同期配送 (Outbox)。
