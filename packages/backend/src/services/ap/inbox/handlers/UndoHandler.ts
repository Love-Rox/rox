...
export class UndoHandler extends BaseHandler {
  ...
  private async handleUndoFollow(
    ...
    this.log(
      "📥",
      `Undo Follow: ${remoteActor.username}@${remoteActor.host} → recipient ${recipientId}`,
    );
    ...
    this.log(
      "✅",
      `Follow deleted: ${remoteActor.username}@${remoteActor.host} unfollowed recipient`,
    );
   ...
   * Handle Undo Like (unlike)
   */
  private async handleUndoLike(object: any, remoteActor: any, c: any): Promise<HandlerResult> {
    ...
    this.log(
      "✅",
      `Reaction deleted: ${remoteActor.username}@${remoteActor.host} unliked note ${note.id}`,
    );
   ...
   * Handle Undo Announce (unboost/unrenote)
   */
  private async handleUndoAnnounce(object: any, remoteActor: any, c: any): Promise<HandlerResult> {
