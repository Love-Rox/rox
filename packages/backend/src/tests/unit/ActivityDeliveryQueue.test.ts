...
import { ActivityDeliveryQueue, JobPriority, type DeliveryJobData } from '../../services/ap/ActivityDeliveryQueue';
...
describe('ActivityDeliveryQueue', () => {
  ...
  describe('enqueue', () => {
    ...
    test('should handle multiple enqueue calls', async () => {
      ...
      const promises = [
        queue.enqueue({ ...baseJobData, inboxUrl: 'https://server1.example/inbox', activity: { ...baseJobData.activity, id: '1' } }),
        queue.enqueue({ ...baseJobData, inboxUrl: 'https://server2.example/inbox', activity: { ...baseJobData.activity, id: '2' } }),
        queue.enqueue({ ...baseJobData, inboxUrl: 'https://server3.example/inbox', activity: { ...baseJobData.activity, id: '3' } }),
      ];
...
describe('ActivityDeliveryQueue rate limiting', () => {
  test('should extract hostname from inbox URL', () => {
    ...
    expect(extractHostname('https://remote.example.com/users/alice/inbox')).toBe('remote.example.com');
