...
describe("Admin Middleware", () => {
  ...
  describe("requireAuth with suspension check", () => {
    ...
    test("should block suspended users with 401 (session invalidated)", async () => {
      ...
      mockUserRepo.findById = mock(() => Promise.resolve(createMockUser({ isSuspended: true })));
  ...
  describe("requireAdmin", () => {
    test("should allow admin users", async () => {
      mockUserRepo.findById = mock(() => Promise.resolve(createMockUser({ isAdmin: true })));
    ...
    test("should reject non-admin users with 403", async () => {
      mockUserRepo.findById = mock(() => Promise.resolve(createMockUser({ isAdmin: false })));
