...
describe("crawlerDetection", () => {
  describe("isEmbedCrawler", () => {
    ...
    it("should detect Slack link expanding bot", () => {
      expect(isEmbedCrawler("Slackbot-LinkExpanding 1.0 (+https://api.slack.com/robots)")).toBe(
        true,
      );
    });

    it("should detect Slack image proxy bot", () => {
      expect(isEmbedCrawler("Slack-ImgProxy 0.19 (+https://api.slack.com/robots)")).toBe(true);
    });
    ...
    it("should detect Facebook external hit bot", () => {
      expect(
        isEmbedCrawler("facebookexternalhit/1.1 (+http://www.facebook.com/externalhit_uatext.php)"),
      ).toBe(true);
    });

    it("should detect LinkedIn bot", () => {
      expect(isEmbedCrawler("LinkedInBot/1.0 (compatible; Mozilla/5.0;)")).toBe(true);
    });
    ...
    it("should not detect Google bot (not an embed crawler)", () => {
      expect(
        isEmbedCrawler("Mozilla/5.0 (compatible; Googlebot/2.1; +http://www.google.com/bot.html)"),
      ).toBe(false);
    });

    it("should not detect Bing bot (not an embed crawler)", () => {
      expect(
        isEmbedCrawler("Mozilla/5.0 (compatible; bingbot/2.0; +http://www.bing.com/bingbot.htm)"),
      ).toBe(false);
    });
  });
  ...
  describe("isActivityPubRequest", () => {
    ...
    it("should return true when activity+json is part of Accept header", () => {
      expect(
        isActivityPubRequest("application/activity+json, application/ld+json, text/html"),
      ).toBe(true);
    });

    it("should return true when ld+json is part of Accept header", () => {
      expect(isActivityPubRequest("text/html, application/ld+json")).toBe(true);
    });
  });
});
